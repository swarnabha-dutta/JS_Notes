...
formSelect.addEventListener("submit", (e) => {
    ...
    if (heightSelect < 0 || heightSelect === "" || isNaN(heightSelect)) {
        ...
        // console.log(bmi);
        if (bmi <= 18.6) {
            resultsSelect.textContent = `Your BMI is ${bmi} and you are Under Weight`;
        } else if (bmi > 18.6 && bmi <= 24.9) {
            resultsSelect.textContent = `Your BMI is ${bmi} and you are Normal Weight`;
        } else {
            resultsSelect.textContent = `Your BMI is ${bmi} and you are Over Weight`;
        }
    }
