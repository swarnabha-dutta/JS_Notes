...
formSelect.addEventListener("submit", (e) => {
    ...
    if (heightSelect < 0 || heightSelect === "" || isNaN(heightSelect)) {
        resultsSelect.textContent = "Please enter a valid height";
    } else if (weightSelect < 0 || weightSelect === "" || isNaN(weightSelect)) {
        resultsSelect.textContent = "Please enter a valid weight";
    } else {
        const bmi = (weightSelect / ((heightSelect / 100) * (heightSelect / 100))).toFixed(2);
        resultsSelect.textContent = bmi;
    }
